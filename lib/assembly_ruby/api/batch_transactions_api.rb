...
module AssemblyRuby
  class BatchTransactionsApi
    ...
    def show_batch_transaction_items_with_http_info(id, opts = {})
      ...
      if @api_client.config.client_side_validation && !opts[:'limit'].nil? && opts[:'limit'] > 200
        fail ArgumentError, 'invalid value for "opts[:"limit"]" when calling BatchTransactionsApi.show_batch_transaction_items, must be smaller than or equal to 200.'
      end

      if @api_client.config.client_side_validation && !opts[:'limit'].nil? && opts[:'limit'] < 1
        fail ArgumentError, 'invalid value for "opts[:"limit"]" when calling BatchTransactionsApi.show_batch_transaction_items, must be greater than or equal to 1.'
      end

      if @api_client.config.client_side_validation && !opts[:'offset'].nil? && opts[:'offset'] < 0
        fail ArgumentError, 'invalid value for "opts[:"offset"]" when calling BatchTransactionsApi.show_batch_transaction_items, must be greater than or equal to 0.'
      end
